...
export function createOutsideScene(){
    k.scene("outside", async (data) => {
        ...
        const map = k.add([k.sprite("mapTwo"), k.pos(0), k.scale(scaleFactor)]);
        ...
        for (const layer of layers) {
          ...
          if(boundary.name === "home"){
            ...
            if(boundary.name === "grocery"){
            player.onCollide("grocery",()=>{
            ...
            if(boundary.name === "BIS"){
              player.onCollide("BIS",()=>{
        ...
        // for (const layer of layers) {
            if (layer.name === "spawnpoints") {
                for (const entity of layer.objects) {
                    if (entity.name === "spawn-home" && previousScene === "home") {
                        player.pos = k.vec2(
                            (map.pos.x + entity.x) * scaleFactor,
                            (map.pos.y + entity.y) * scaleFactor
                        );
                        k.add(player);
                    } else if (entity.name === "spawn-castle" && previousScene === "grocery") {
                        player.pos = k.vec2(
                            (map.pos.x + entity.x) * scaleFactor,
                            (map.pos.y + entity.y) * scaleFactor
                        );
                        k.add(player);
                    }else if (entity.name === "spawn-bis" && previousScene === "workshop") {
                      player.pos = k.vec2(
                          (map.pos.x + entity.x) * scaleFactor,
                          (map.pos.y + entity.y) * scaleFactor
                      );
